...
function BiometricGate({onPass,onSkip}){
  ...
  const CRED_KEY="us_bio_cred";
  const storedId=gs(CRED_KEY,null);
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(!avail){setStatus("unsupported");return;}
      if(storedId){await doAuth(storedId);}
      else{setStatus("register");}
    })();
  },[]);

  async function doRegister(){
    setStatus("registering");setMsg("");
    try{
      const challenge=window.crypto.getRandomValues(new Uint8Array(32));
      const cred=await navigator.credentials.create({publicKey:{
        challenge,
        rp:{name:"us.",id:window.location.hostname},
        user:{id:new Uint8Array([1]),name:"us.user",displayName:"us."},
      ...
      setMsg(e.name==="NotAllowedError"?"Setup cancelled — try again.":e.message);
      setStatus("register");
    }
  }

  async function doAuth(id){
    setStatus("authing");setMsg("");
    try{
      const challenge=window.crypto.getRandomValues(new Uint8Array(32));
      const rawId=new Uint8Array(atob(id).split("").map(c=>c.charCodeAt(0)));
      ...
      setMsg(e.name==="NotAllowedError"?"Biometric cancelled.":e.message);
      setStatus("retry");
    }
  }

  const wrap=(ico,title,body)=>(
    <div className="nfc-wrap">
      <div style={{fontSize:"clamp(44px,12vw,56px)",marginBottom:16,animation:"hb 2s ease infinite"}}>{ico}</div>
      <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(22px,6vw,28px)",fontWeight:600,color:"var(--ink)",marginBottom:10,textAlign:"center"}}>{title}</h2>
      <p style={{fontSize:14,color:"var(--muted)",textAlign:"center",lineHeight:1.75,maxWidth:280,marginBottom:22}}>{body}</p>
      {msg&&<p style={{color:"#e05050",fontSize:13,textAlign:"center",marginBottom:14,maxWidth:260}}>{msg}</p>}
    </div>
  );

  if(status==="unsupported")return(
    <div className="nfc-wrap">
      {wrap("🔐","Biometric unavailable","This browser or device doesn't support biometric unlock.")}
      <button className="btn-p" style={{maxWidth:260,width:"100%"}} onClick={onSkip}>Continue →</button>
    </div>
  );
  if(status==="register")return(
    <div className="nfc-wrap">
      {wrap("👆","Set up biometric","Add Face ID or fingerprint as a second lock after your NFC tag.")}
      <button className="btn-p" style={{maxWidth:260,width:"100%",marginBottom:12}} onClick={doRegister}>Set Up Biometric</button>
  ...
  if(status==="retry")return(
    <div className="nfc-wrap">
      {wrap("🔐","Try again","")}
      <button className="btn-p" style={{maxWidth:260,width:"100%",marginBottom:12}} onClick={()=>doAuth(storedId)}>Retry</button>
      <button className="btn-g" style={{marginBottom:8}} onClick={()=>{ss(CRED_KEY,null);setStatus("register");}}>Reset &amp; Re-register</button>
      <button className="btn-g" onClick={onSkip}>Skip for now</button>
    </div>
  );
  return(
    <div className="nfc-wrap">
      ...
      <p style={{marginTop:14,fontSize:13,color:"var(--muted)"}}>
        {status==="registering"?"Setting up biometric…":"Verifying…"}
      </p>
    </div>
  );
}
...
function SettingsPage({pageName,setPageName,theme,setTheme,bgImage,setBgImage,names,setNames,
  myName,theirName,startDate,onSettings,fbApiKey,setFbApiKey,fbDbUrl,setFbDbUrl,synced,onReplayIntro}){
